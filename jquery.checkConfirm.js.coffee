  ...
  $.fn.checkConfirm = (options) ->
    ...
    $(document).on
      keyup: (doc) ->
        ...
        # If the element is the confirm password
        else

          # If the length is equal to the length and matches the original password
          if checkLength(el) and that[0].value is el.value
            validatePass el
